  ...
  const arrayMethodFixes = [
    {
      ...
      replacement: '.map((assignment: { id: string; title: string; description?: string; dueDate?: Date; status: string; submissions?: any[] }) =>'
    },
    {
      pattern: /\.map\((sub)\s*=>/g,
      replacement: '.map((sub: { studentId: string; answers?: any[] }) =>'
    },
    {
      pattern: /\.map\((ans)\s*=>/g,
      replacement: '.map((ans: { id: string; text: string; isCorrect?: boolean }) =>'
    },
    {
      pattern: /\.map\((answer)\s*=>/g,
      replacement: '.map((answer: { id: string; text: string; isCorrect?: boolean }) =>'
    },
    {
      pattern: /\.map\((submission)\s*=>/g,
      replacement: '.map((submission: { id: string; studentId: string; answers?: any[] }) =>'
    },
    {
      pattern: /\.map\((classItem)\s*=>/g,
      replacement: '.map((classItem: { id: string; name: string }) =>'
    },
    {
      pattern: /\.filter\((user)\s*=>/g,
      replacement: '.filter((user: { id: string; teacherId?: string; studentId?: string }) =>'
    }
  ];
